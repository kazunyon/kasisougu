...
| 近くで探す | 都道府県・市区町村、移動手段、移動可能時間、目的から、確認済みの相談先を最大5件表示。初期候補は埼玉県の公式情報を登録 |
...
都道府県を選ぶと市区町村の候補を切り替え、移動手段（車・公共交通・福祉タクシー・電動車いす）、移動可能時間（30・60・90分）、目的（装具の製作・修理・適合確認・リハビリ・制度相談）で候補を絞ります。表示は最大5件です。

初期版は、公式ページを確認した埼玉県内の候補だけを掲載しています。都道府県・市区町村の選択肢は検索画面の操作用の初期候補であり、全国の全市区町村・全施設を網羅するものではありません。移動時間は経路検索結果ではなく登録した目安です。予約の要否、対応内容、実際の経路・バリアフリー状況は、表示された公式ページと施設へ必ず確認してください。
## 構築と公開
...
オフライン下書きと自動再送、完全書き出し・取込復元・アカウント削除、記事レビュー／公開管理、全国の施設データ・経路検索APIとの連携は未実装です。「近くで探す」は埼玉県の公式情報を用いた初期候補の絞り込みです。監査・再送管理用テーブルがあっても現行画面からは利用していません。
