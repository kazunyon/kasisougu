...
2. 「検索する住所」に探したい地域の住所を入力します。現在地の周辺を探す場合は「現在地を使用」を押し、位置情報の利用を許可します。
3. 「リハビリを探す」「整形外科を探す」「義肢装具店を探す」「装具外来を探す」など、探したい施設のボタンを押します。
4. 別の画面で開いたGoogle Mapsで、施設情報や経路を確認します。必要に応じて地図の範囲や検索語を変更します。
...
3. 「候補施設を保存」を押します。
4. 保存した施設を見るときは、「登録施設から探す」タブへ切り替え、「直線距離を調べる」を押して検索します。
...
5. 移動経路を調べたい施設の「Google Mapsで経路を確認」を押します。
