...
都道府県を選ぶと、ブラウザから読める全国住所データで該当都道府県の市区町村候補を読み込みます。目的（装具の製作・修理・適合確認・リハビリ・制度相談）と地域に一致する、`kasi_nearby_facilities` の有効な登録施設を取得し、出発地からの直線距離が近い順に表示します。現在地をFROMにする場合は端末の位置情報を使います。登録住所をFROMにする場合は、本人専用設定の住所を国土地理院の住所検索で緯度経度へ変換します。
