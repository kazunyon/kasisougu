...
| 近くで探す | 登録施設を目的・距離で絞り、現在地／入力住所から近い順に全件表示。Google Mapsの外部リンクで経路を確認 |
...
現在地または入力した出発地（本人設定に保存）をFROMにし、データベース `kasi_nearby_facilities` に登録された施設をTOとして直線距離を端末内で計算します。目的と範囲（10・30・50km／距離制限なし）で絞り、近い順に全件表示します。100件ずつ最後まで取得してから並べ替えるため、名前順の先頭だけを距離順にすることはありません。未登録と取得失敗は別々に案内します。

「Google Mapsで経路を確認」はAPIキー不要の [Maps URL](https://developers.google.com/maps/documentation/urls/get-started) を開きます。道路距離・所要時間・車／徒歩／公共交通の経路はGoogle Maps側で確認します。Google Maps Platform API・OpenStreetMap・Overpassによる施設検索や経路計算は行いません。Google API料金は発生しませんが、既存のDB・通信費は別です。
...
施設は運営者が [施設登録手順](docs/nearby-facilities.md) に従ってDBへ追加します。施設名・住所・緯度経度・対応目的は公式情報等で確認してください。固定3件のコード上の代替表示は廃止しました。既存の施設マスタがあればスキーマ変更は不要です。未構築環境は [施設マスタSQL](supabase/migrations/20260919100000_nearby_facilities.sql) を適用します。過去の3件登録SQLは参考履歴として残しますが、内容を再確認せずに適用しないでください。
...
オフライン下書きと自動再送、完全書き出し・取込復元・アカウント削除、記事レビュー／公開管理は未実装です。「近くで探す」の施設網羅性は運営者が登録した施設マスタに依存し、バリアフリー・診療や製作対応を保証しません。施設管理画面は未実装です。監査・再送管理用テーブルがあっても現行画面からは利用していません。
