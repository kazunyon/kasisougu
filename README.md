...
共通施設は運営者が [施設登録手順](docs/nearby-facilities.md) に従ってDBへ追加します。利用者はGoogle Maps等で見つけた施設を本人専用候補として手動登録でき、「登録施設から探す」の候補施設カードから編集・削除できます。削除は論理削除です。施設名・施設種別・確認日は必須で、所在地等は任意です。候補施設保存には [候補施設SQL](supabase/migrations/20260920100000_candidate_facilities.sql) の適用が必要です。
...
オフライン下書きと自動再送、完全書き出し・取込復元・アカウント削除、記事レビュー／公開管理は未実装です。「登録施設から探す」の共通施設の網羅性は運営者が登録した施設マスタに依存し、Google Mapsの表示内容を含め、バリアフリー・診療や製作対応を保証しません。共通施設の管理画面は未実装です。監査・再送管理用テーブルがあっても現行画面からは利用していません。
