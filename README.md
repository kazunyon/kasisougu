...
F05は `kasi_consultation_sheets.snapshot_json` に選択対象と表示内容を保存します。確定時には選択写真を本人専用Storage内の相談シート用パスへコピーするため、元の記録・困りごと・写真を後から変更しても、確定済みの表示は変わりません。確定済みシートで「内容を編集する」を選ぶと、内容を引き継ぐ新しい下書きを作成できます。下書きでは相談日・相談先・質問に加え、装具・困りごと・記録・写真の選択も追加・削除できます。相談シートの編集画面では、関連する登録済み装具を指定して新しい写真をアップロードでき、追加直後に掲載写真として選択されます。各シートは一覧の「削除」から確認後に削除できます。確定版用に複製した写真は、このとき一緒に削除します。印刷はブラウザーのA4縦用CSSを使用します。既存の関連テーブルは、選択内容の保持には使用していません。
