...
F04の使用記録は、現在使用中の装具に紐づく最新の通常記録を最初に表示します。過去分・比較用も選択欄から開けます。「記録を保存」は選択中の記録を更新し、「比較用として別に保存」は入力欄の内容と8項目評価を別レコードへ保存します（写真は複製しません）。比較用は `record_kind = comparison` で区別し、元の記録の更新では変わりません。既存の記録は通常記録のまま保持します。

この改修の公開前に [F04保存マイグレーション](supabase/migrations/20260915160000_f04_current_comparison_records.sql) を適用してください。RPC `kasi_save_usage_record` は本人のRLS権限で動き、記録と評価を同じトランザクションで保存します。未適用では新しい画面の保存はできません。ローカルの画面検証は `tools/check_redesign.js` の後に `tools/check_current_records.js` を実行します。DBへの適用と実DBのRLS・ロールバック検証は別途必要です。
...
F05は `kasi_consultation_sheets.snapshot_json` に選択対象と表示内容を保存します。確定時には選択写真を本人専用Storage内の相談シート用パスへコピーするため、元の記録・困りごと・写真を後から変更しても、確定済みの表示は変わりません。確定済みシートで「内容を編集する」を選ぶと、内容を引き継ぐ新しい下書きを作成できます。下書きでは相談日・相談先・質問に加え、装具・困りごと・記録・写真の選択も追加・削除できます。元の確定版は履歴としてそのまま残ります。印刷はブラウザーのA4縦用CSSを使用します。既存の関連テーブルは、選択内容の保持には使用していません。
...
使用記録の「開く記録」には、保存済みの使用記録に加え、まだ使用記録がない登録装具を「記録未入力」として表示します。選択するとその装具の入力欄が開き、保存時に初めて使用記録を作成します。同名の装具は選択肢内で番号を付けて区別します。比較欄には保存済みの記録のみ表示します。`tools/check_unrecorded_orthoses.js` は `tools/check_redesign.js` の後に実行し、装具5件（試用中3件）・記録2件のケースを確認します。

使用記録の「削除」は確認後に論理削除します。保存済みの記録では使用記録だけを削除し、装具は残します。記録未入力の候補では登録装具を削除するため「自分の装具」からも除外します。削除直前に保存済み記録の有無を確認し、更新版数が一致しない場合は削除しません。写真や評価の物理削除は行いません。検証は `tools/check_redesign.js` の後に `tools/check_record_deletion.js` を実行します。
