...
| 近くで探す | 登録済み施設を都道府県・市区町村・目的で絞り、現在地または登録住所からの直線距離が近い順に表示。道路距離・所要時間はGoogle Mapsで確認 |
...
現在地または登録住所を出発地にし、検索ボタンを押した時点の OpenStreetMap 公開データから、義肢装具・補装具・障害者支援・リハビリ・病院の候補を取得します。目的と検索範囲（10・30・50km）を選べ、出発地からの直線距離が近い順に最大5件を表示します。現在地をFROMにする場合は端末の位置情報を使います。登録住所をFROMにする場合は、本人専用設定の住所を国土地理院の住所検索で緯度経度へ変換します。公開地図サービスが一時的に応答しない場合でも、確認済みの施設マスタ候補は表示を試みます。

各候補の「地図で経路を確認」は、出発地と施設の座標を指定した Maps URL を開きます。道路距離・所要時間・交通手段・道順は地図側で確認します。Maps URLにはAPIキーは不要です。Google Maps Platform の Places API・Routes API・Maps JavaScript API は使用しません。OpenStreetMap の公開データは更新状況や登録状況によって候補の網羅性が変わります。道路・運行・バリアフリー・予約・製作や修理の可否は判定しないため、必ず施設へ事前に確認してください。
