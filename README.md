...
F03の分類は `kasi_catalog_terms` の `support_scope.code` を使い、表示名の部分一致では判定しません。検索は公開済み記事の装具名・製品名・概要と5分類のラベル・説明を対象とし、分類フィルターと併用できます。詳細と比較では未登録の値を「未確認」と表示し、重さ・着脱・靴との相性などを概要文から推測しません。出典の確認日は `kasi_catalog_sources.checked_on` を表示します。比較は最大3件で、優劣や適合性の判定はしません。素材・継手・足元構造・特徴の表示を充実させるには、専門職が確認した分類データの登録が必要です。
