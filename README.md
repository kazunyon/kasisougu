...
3. 「Google Mapsで目的地を選ぶ」を押します。
4. 入力欄の住所（通常は登録住所）だけが出発地に入ったGoogle Mapsの経路画面が開きます。
5. Google Mapsの目的地欄で施設名や種類を検索し、表示された候補から行きたい施設を選びます。アプリが目的地を自動決定することはありません。現在地を出発地にする場合だけ、先に「現在地を使用」を選びます。

住所は保存しなくてもGoogle Mapsの検索に使えます。次回も使う場合は「この住所を保存」を押します。Google Mapsが開かないときは、表示される「Google Mapsの経路をブラウザで開く」リンクを押してください。
