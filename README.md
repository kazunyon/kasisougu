...
都道府県を選ぶと、全国住所データから該当都道府県の全市区町村候補を読み込みます。移動手段（車・公共交通・福祉タクシー・電動車いす）、移動可能時間（30・60・90分）、目的（装具の製作・修理・適合確認・リハビリ・制度相談）を選ぶと、Google Maps PlatformのPlaces検索と経路検索で候補を絞り、最大5件を表示します。アプリ内には施設マスタを保存しません。

経路の出発地点は選択した市区町村の代表地点です。福祉タクシーは車、電動車いすは徒歩の経路を参考表示します。道路・運行・バリアフリー・予約・製作や修理の可否は判定しないため、必ず施設へ事前に確認してください。

Google Cloudで **Maps JavaScript API** と **Places API (New)** を有効にし、HTTPリファラーで公開サイトに制限したブラウザ用APIキーをGitHub ActionsのRepository variable `GOOGLE_MAPS_API_KEY` に設定してください。このキーはブラウザで利用するため、secret keyやサービスアカウントキーは設定しません。キーがない場合、近くで探す画面は設定案内を表示します。
## 構築と公開
