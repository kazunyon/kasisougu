...
| 近くで探す | 全国の都道府県・市区町村、移動手段、移動可能時間、目的から、Google Mapsの検索結果を最大5件表示 |
...
都道府県を選ぶと、ブラウザから読める全国住所データで該当都道府県の全市区町村候補を読み込みます。移動手段（車・公共交通・福祉タクシー・電動車いす）、移動可能時間（30・60・90分）、目的（装具の製作・修理・適合確認・リハビリ・制度相談）を選ぶと、Google Maps PlatformのPlaces検索と経路検索で候補を絞り、最大5件を表示します。住所（任意）は本人専用データとして保存され、入力済みなら経路の出発地に使います。各候補のGoogle Mapsリンクは、出発地から施設までの経路を開きます。アプリ内には施設マスタを保存しません。

経路の出発地点は選択した市区町村です。福祉タクシーは車、電動車いすは徒歩の経路を参考表示します。道路・運行・バリアフリー・予約・製作や修理の可否は判定しないため、必ず施設へ事前に確認してください。

Google Cloudで **Maps JavaScript API**、**Places API (New)**、**Routes API** を有効にし、HTTPリファラーで公開サイトに制限したブラウザ用APIキーをGitHub ActionsのRepository variable `GOOGLE_MAPS_API_KEY` に設定してください。このキーはブラウザで利用するため、secret keyやサービスアカウントキーは設定しません。キーがない場合、近くで探す画面は設定案内を表示します。
### パスワードを忘れた場合
...
$env:GOOGLE_MAPS_API_KEY='ブラウザ用・HTTPリファラー制限済みのAPIキー'
...
オフライン下書きと自動再送、完全書き出し・取込復元・アカウント削除、記事レビュー／公開管理は未実装です。「近くで探す」はGoogle Mapsの検索・経路結果に依存し、施設の網羅性・バリアフリー・診療や製作対応を保証しません。監査・再送管理用テーブルがあっても現行画面からは利用していません。
