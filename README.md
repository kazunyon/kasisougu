...
| 近くで探す | 登録住所／別の住所／現在地の周辺施設を10種類の目的別Google Maps検索で探し、候補施設を手動登録 |
...
「近くで探す」はGoogle Maps検索専用です。登録住所・別の住所または許可後に取得した現在地と、10種類の検索語をMaps検索URLの `query` に設定します。Google Maps URL方式では検索半径を厳密に指定できないため距離選択は設けず、地図上の「このエリアを検索」で調整するよう案内します。検索結果をアプリ内へ取得せず、施設マスタの一覧表示や直線距離計算も行いません。

検索はAPIキー不要の [Maps URL](https://developers.google.com/maps/documentation/urls/get-started) を開きます。道路距離・所要時間・車／徒歩／公共交通の経路、施設情報、口コミ等はGoogle Maps側で確認します。Google Maps Platform API・OpenStreetMap・Overpassによる施設検索や経路計算は行いません。Google API料金は発生しませんが、既存のDB・通信費は別です。

保存する住所の位置確認には国土地理院の住所検索を使用します。入力住所は同サービスへ、検索リンクを開く際には住所・検索語または現在地の座標がGoogle Mapsへ送信されます。現在地は保存しません。

利用者はGoogle Maps等で見つけた施設を本人専用候補として手動登録できます。施設名・施設種別・確認日は必須で、所在地等は任意です。候補施設保存には [候補施設SQL](supabase/migrations/20260920100000_candidate_facilities.sql) の適用が必要です。未適用の場合、アプリは日本語で管理者への適用依頼を案内します。
...
オフライン下書きと自動再送、完全書き出し・取込復元・アカウント削除、記事レビュー／公開管理は未実装です。Google Mapsの表示内容について、バリアフリー・診療や製作対応を保証しません。本人候補の編集・削除画面は未実装です。過去の施設マスタ用マイグレーションは履歴として残しますが、現行画面からは使用しません。
