...
async (page) => {
  ...
  await page.getByRole('button',{name:'Google Mapsで目的地を選ぶ',exact:true}).click();
  ...
  assert(openedRoute.origin.endsWith('堀崎町1592'),'Google Maps search must not truncate the street number from its origin');
  assert(openedRoute.destination === null,'Google Maps destination must stay empty for the user to choose');
  assert(openedRoute.travelmode === 'walking','Google Maps search must default to walking');
  await page.screenshot({path:'output/playwright/nearby-destination-choice.png',fullPage:true});
