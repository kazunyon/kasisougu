...
(async () => {
  ...
  assert.equal(searchUrl.searchParams.get('origin'),'さいたま市見沼区堀崎町1592');
  assert.equal(searchUrl.searchParams.get('destination'),'リハビリテーション科 さいたま市見沼区堀崎町1592');
  ...
  assert.equal(currentSearchUrl.searchParams.get('destination'),'整形外科 35.9,139.6');
  ...
  console.log('PASS: distance, pagination, candidate merge, explicit route origins, 10 URL searches, encoding, empty/error states, and no paid Maps API.');
