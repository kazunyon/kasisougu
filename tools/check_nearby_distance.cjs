...
(async () => {
  ...
  const searchUrl = new URL(run("nearbyMapsSearchUrl('リハビリテーション科',{address:'埼玉県さいたま市見沼区堀崎町1592'})"));
  assert.equal(searchUrl.searchParams.get('api'),'1');
  assert.equal(searchUrl.searchParams.get('query'),'リハビリテーション科 埼玉県さいたま市見沼区堀崎町1592');
  assert.equal(searchUrl.searchParams.get('origin'),null);
  assert.equal(searchUrl.searchParams.get('destination'),null);
  const currentSearchUrl = new URL(run("nearbyMapsSearchUrl('整形外科',{lat:35.9,lng:139.6})"));
  assert.equal(currentSearchUrl.searchParams.get('query'),'整形外科 35.9,139.6');
  ...
  console.log('PASS: distance, pagination, candidate merge, exact address search, 10 facility buttons, no automatic destination, encoding, empty/error states, and no paid Maps API.');
