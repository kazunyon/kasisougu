...
function nearbyRenderResults(items, origin) {
  ...
  items.forEach((item,index) => {
    ...
    if (!item.isCandidate && /^https?:\/\//i.test(item.website_url || '')) { const website=document.createElement('a'); website.className='resource-link'; website.href=item.website_url; website.target='_blank'; website.rel='noopener noreferrer'; website.textContent='施設サイトを開く'; links.append(website); }
