...
async function nearbyFindFacilities(origin, condition) {
  ...
  let candidates = [];
  try { candidates = await nearbySelectAll('kasi_candidate_facilities','id,name,address,latitude,longitude,phone,google_maps_url,facility_type,checked_on',{deleted_at:'is.null'}); } catch { candidates = []; }
  const facilities = [];
...
function nearbyDirectionsUrl(origin, item) { const destination = item.location ? `${item.location.lat},${item.location.lng}` : item.address; return `https://www.google.com/maps/dir/?${new URLSearchParams({api:'1',origin:`${origin.lat},${origin.lng}`,destination}).toString()}`; }
...
async function nearbySearch() {
  const condition={address:nearbySavedAddress,purpose:nearby$('nearby-purpose').value,radius:Number(nearby$('nearby-radius').value),origin:document.querySelector('input[name="nearby-origin"]:checked')?.value}; const button=nearby$('nearby-search'); button.disabled=true; nearby$('nearby-results').replaceChildren(); nearbySetStatus('登録施設までの直線距離を確認しています…');
  try { const origin=await nearbyOrigin(condition); const facilities=await nearbyFindFacilities(origin,condition); nearbyRenderResults(facilities,origin); nearbySetStatus(`${origin.label}から、条件に合う登録施設を近い順に${facilities.length}件表示しています。距離は直線距離です。`); } catch(error){ nearby$('nearby-results').replaceChildren(); nearbySetStatus(error.message||'検索できませんでした。',true); } finally{ button.disabled=false; }
}
...
async function nearbySaveCandidate(event) {
  ...
  try { const value=id=>nearby$(id).value.trim(), address=value('candidate-address'); let location=null, locationWarning=''; if(address){ try{ location=await nearbyGeocodeAddress(address); }catch{ locationWarning=' 所在地の位置を確認できなかったため、直線距離は表示されません。'; } }
    ...
    if(!body.name||!body.facility_type||!body.checked_on) throw new Error('施設名、施設種別、確認日を入力してください。'); if(body.google_maps_url && !/^https?:\/\//i.test(body.google_maps_url)) throw new Error('Google Maps URLは http:// または https:// から入力してください。'); await window.KASI_API.insert('kasi_candidate_facilities',body); form.reset(); nearby$('candidate-checked-on').value=nearbyToday(); nearbySetStatus(`候補施設を保存しました。登録施設から探す対象に追加されました。${locationWarning}`,false,'candidate-status');
  } catch(error){ nearbySetStatus(error.message||'候補施設を保存できませんでした。',true,'candidate-status'); } finally{ button.disabled=false; }
}
...
async function nearbyInit() {
  ...
  nearby$('nearby-address-cancel').addEventListener('click',()=>{nearby$('nearby-address').value=nearbySavedAddress; nearbySetStatus('住所の変更を取り消しました。',false,'nearby-maps-status');}); nearby$('nearby-candidate-toggle').addEventListener('click',()=>nearbyToggleCandidate(nearby$('nearby-candidate-form').hidden)); nearby$('candidate-cancel').addEventListener('click',()=>{nearby$('nearby-candidate-form').reset();nearbyToggleCandidate(false);}); nearby$('nearby-candidate-form').addEventListener('submit',nearbySaveCandidate); try{ await nearbyLoadSavedAddress(); }catch(error){ nearbySetStatus(`保存した住所を読み込めませんでした：${error.message}`,true,'nearby-maps-status'); }
}
