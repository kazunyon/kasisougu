-- Restore a complete user backup without deleting existing data.
-- The browser uploads photo objects first, then this RPC restores all database rows atomically.
begin;

create table if not exists private.kasi_backup_restores (
...
revoke all on table private.kasi_backup_restores from public, anon, authenticated;
...
begin
  ...
  for item in select * from jsonb_array_elements(backup_data->'concerns') loop
    insert into public.kasi_usage_record_concerns(
      id, owner_id, usage_record_id, noted_on, category_code, description, occurred_timing,
      status_code, action_note, resolved_on, created_at, updated_at
    ) values (
      (item->>'id')::uuid, current_user_id, (item->>'usage_record_id')::uuid, (item->>'noted_on')::date,
